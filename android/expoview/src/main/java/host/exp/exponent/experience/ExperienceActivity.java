...
public class ExperienceActivity extends BaseExperienceActivity implements Exponent.StartReactInstanceDelegate {
  ...
  protected void onResume() {
    ...
    if (mDevMenuManager.isShownInActivity(this)) {
      mKernel.getReactInstanceManager().onHostResume(this);
    }
  ...
  /**
   * Handles `adb shell input keyevent 82` command to toggle DevMenu
   * on the current experience activity from the command line.
   */
  @Override
  ...
  /**
   * Closes DevMenu when pressing back button when it is visible on this activity.
   */
  @Override
